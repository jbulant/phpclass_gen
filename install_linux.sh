...
then
	...
	find2=$?;
	if [ $find2 -le 0 ] || [ $find1 -le 0 ]
	then
		grep "phpcg" ~/.zshrc 2>/dev/null >/dev/null
		find=$?;
		if [ -f ~/.bashrc ] && [ $find -gt 0 ]
		then
...
then
	...
	grep "phpcg" ~/.bashrc 2>/dev/null >/dev/null
	ret=$?;
	if [ -f ~/.bashrc ] && [ $ret -eq 1 ]
	then
		echo "alias phpcg='`realpath $1`/php -f phpclass_gen.php'" >> ~/.bashrc
	...
	if [ -f ~/.zshrc ] && [ $ret == 1 ]
	then
		echo "alias phpcg='`realpath $1`/php -f phpclass_gen.php'" >> ~/.zshrc
		cp phpclass_gen.php $1 2>/dev/null >/dev/null
		cp ClassName.class.php $1 2>/dev/null >/dev/null
		source ~/.zshrc 2>/dev/null >/dev/null
		echo "Install Complete"
